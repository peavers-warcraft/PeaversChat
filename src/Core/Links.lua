...
-- click, Ctrl+C, Escape.
--
-- This uses ChatFrame_AddMessageEventFilter, which is the documented way to
-- alter a chat message and what most addons use.
--
-- It was twice replaced with something more elaborate - a wrapper on each chat
-- frame's AddMessage, then a stand-in frame the client composed against - on
-- the strength of one observation: chat recovering in a Mythic+ when the filter
-- was removed. That observation was confounded. The command that removed the
-- filter also switched off the channel abbreviations, and the channel
-- abbreviations were the actual cause. This was never at fault, and the two
-- replacements were machinery built to avoid a problem that was not here.
--
...
--- Kept out of Initialize, like every other hook in this addon: loading with
--- the addon switched off must install nothing at all, because hooksecurefunc
--- cannot be undone afterwards.
function Links:InstallHooks()
